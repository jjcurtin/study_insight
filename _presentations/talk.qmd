...
- Predictor: most recent self-monitoring score (single predictor, no feature engineering)
...
## Results: Accuracy of dynamic self-monitoring

median ROC = 0.796 (round to 2 decimals so 0.80)

figure - ROC curve? plot histogram of outer loop ROCs? posteriors around auROC? talk to john

## Takeaways?

decide if this should be here or just move into comparison (purpose 2)

if here - maybe brief recap of what this auROC translates to, establishes that people can predict their next-week drinking relatively accurately. not sure if it makes sense yet here to talk about cued, contextualized, etc - maybe better for after comparison w AASE model
...
start with just AASE performance (mirror however dynamic model was displayed)

then model comparisons (bayesian)
